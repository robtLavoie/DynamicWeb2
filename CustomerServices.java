...
public class CustomerServices implements CustomerDAOI  {
	...
	public boolean updateCustomer(Customer cust){
		...
		try {
			// find the customer to update Customer Table
			// and set the attributed from  Session Customer object stored in the session that changes were made
	...
	public boolean deleteCustomer(Customer cust) {
		...
		EntityManager entitymanager = entitymanagerfactory.createEntityManager();
		try {
			//find the customer by email key and  delete
			Customer foundCustomer = entitymanager.find(Customer.class, cust.getcEmail());
			entitymanager.getTransaction().begin();
	...
	public Customer findCustomer(Customer cust) {
		...
		try {
			// usedto find the customer first to perform a function on.
...
// Check to see if customer and password match.		
